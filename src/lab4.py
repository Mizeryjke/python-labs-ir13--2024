class Lamp:
    ...
    def __str__(self):
        return (f"Lamp(type={self.__type}, power={self.__power}W, led_count={self.__led_count}, "
                f"manufacturer={self.__manufacturer}, warranty_period={self.warranty_period}, color={self.color})")

    def __repr__(self):
        return (f"Lamp(type='{self.__type}', power={self.__power}, led_count={self.__led_count}, "
                f"manufacturer='{self.__manufacturer}', warranty_period={self.warranty_period}, color='{self.color}')")

    def __del__(self):
        print(f"Lamp object with type '{self.__type}' and manufacturer '{self.__manufacturer}' is being deleted")

# Основний метод для ініціалізації об'єктів
def main():
